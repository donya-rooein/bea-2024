...
@torch.no_grad()
def rephrasing_text(model, tokenizer, full_prompt, temperature):
    ...
    generated_ids = model.generate(model_inputs, max_new_tokens=256, do_sample=False, temperature=temperature)
...
def main():
    ...
    temperature_values = [0.5, 0.6, 0.8]  # Example temperature values
    ...
    for temperature in temperature_values:
        for index_problems, problem in df_texts.iterrows():
            ...
            column_name = f'rephrase_ll70_temp_{temperature}'  # Define column_name here with temperature value

            try:
                result = rephrasing_text(model, tokenizer, formatted_prompt, temperature)
                ...
                with open(f'error_log_data_ll70_temp_{temperature}.txt', 'a') as error_log_file:
                    error_log_file.write(str(e) + '\n')

    df_texts.to_csv('rephrased_training_multi_temp.csv', index=False)
